...
pooled_train_data <- rbind(daily_fluxes_davos_train |> drop_na(), daily_fluxes_lae_train |> drop_na())

#pooling test data both sites
pooled_test_data <- rbind(daily_fluxes_davos_test, daily_fluxes_lae_test)


#model formulation pooled data
...
#train model on pooled trainign data #error
set.seed(1982)
class(pp_pooled)


pooled_mod_cv <- caret::train(GPP_NT_VUT_REF ~ .,
                              data = pp_pooled_baked,
                              method = "knn",
                              trControl = caret::trainControl(method = "cv", number = 10),
                              tuneGrid = data.frame(k = c(2, 5, 10, 15, 20, 25, 30, 40, 60, 100)),
                              metric = "MAE")

ggplot(pooled_mod_cv)
print(pooled_mod_cv)

#Evaluate the model on the test data for both sites
pooled_predictions <- predict(pooled_mod_cv, newdata = pooled_test_data)

# Evaluate the model metrics for the pooled model
pooled_metrics <- eval_model(predictions = pooled_predictions,
                             actuals = pooled_test_data$GPP_NT_VUT_REF)
