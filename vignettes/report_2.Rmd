---
title: "Report Exercise 2 Tidy Data"
...
experiment_data <- read_csv2("1249534_S1_tidy.csv")
...
#We now have the data grouped into the different 53 experiments
...
Now the log response from each row in the csv file can be c analculatedd added to the file with the mutate() command. But this is not what we want. We want to have one log-response-ratio for each experiment, so a total of only 53 log-response-ratios. This can be calculated using the summarize() command and this summarizing the rows of each experiment, then calculating the mean log-response-ratio, which is what we are looking for.
